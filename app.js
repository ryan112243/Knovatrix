...
function publicResourcePanel(catalog) {
  return `<div class="tab-panel public-resource-panel"><div class="rights-banner"><b>來源與使用原則</b><p>本站只索引官方公開頁面，不把公開下載誤認為可重製授權。未確認授權、明示限校內使用或由主辦單位販售的試題，不會複製到 GitHub。</p></div><h4>${catalog.title}</h4><div class="public-resource-grid">${catalog.items.map(item => {
    const content = `<span class="resource-badge ${item.status}">${item.badge}</span><b>${item.title}</b><p>${item.detail}</p><small>${item.url ? "前往原始發布頁面 ↗" : "待主辦單位提供公開授權"}</small>`;
    return item.url ? `<a class="public-resource-card" href="${item.url}" target="_blank" rel="noopener noreferrer">${content}</a>` : `<div class="public-resource-card restricted">${content}</div>`;
  }).join("")}</div></div>`;
}
...
function aboutPage() {
  return `<section class="page-hero"><div class="wrap reveal"><p class="eyebrow">About Knovatrix</p><h1>讓好的學習資源，<br>不再散落各處。</h1><p class="lead">Knovatrix 希望建立一座持續成長、容易使用、由學習者共同維護的台灣 K–12 學科資源庫。</p></div></section><section class="section"><div class="wrap"><div class="info-grid"><article class="info-card"><span class="big-icon">🧭</span><h3>我們在做什麼</h3><p>把常規課綱、資優甄選與競賽資源放進同一張知識地圖，降低尋找資料與判斷程度的成本。</p></article><article class="info-card"><span class="big-icon">🔬</span><h3>內容如何把關</h3><p>核心團隊具科學班學習背景，負責內容校對、補充圖解，並依讀者需求製作解題影片。</p></article><article class="info-card"><span class="big-icon">🌱</span><h3>我們相信共創</h3><p>每次勘誤、筆記投稿與題目補充，都能讓下一位學習者少走一點彎路。</p></article><article class="info-card"><span class="big-icon">🪶</span><h3>保持輕量與開放</h3><p>網站不設複雜會員系統；資源公開瀏覽，意見與共創透過匿名表單分流。</p></article></div><div class="callout"><div><h3>這是一座正在長大的知識庫</h3><p>第一階段先完成架構，內容與團隊故事將持續補上。</p></div><a class="button" href="#/contribute">參與共創 →</a></div></div></section>`;
}
