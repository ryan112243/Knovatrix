...
function scienceExamPanel(schoolName, sourceFiles = null) {
  ...
  return `<div class="tab-panel exam-panel"><div class="exam-source"><div><span>${school.city} · ${school.archiveLabel || "科學班歷屆"}</span><b>${schoolName}</b><p>${school.note || ""}${fileHint}</p></div></div><div class="exam-legend"><span><i class="direct"></i>有檔案</span><span><i class="pending"></i>沒有檔案</span></div><div class="year-grid">${years.map(year => {
    ...
    if (localFiles.length === 1) {
      ...
      const href = file.url || (file.path ? file.path.replace(/\\/g, "/").replace(/\/{2,}/g, "/") : "");
...
function subjectSidebar(subjectNames, id) {
  const level = catalogLevels[id];
...
function supportPage() {
  return `<section class="page-hero"><div class="wrap reveal"><p class="eyebrow">Support the project</p><h1>一起維持知識庫，<br>長久而自由地運作。</h1><p class="lead">你的支持將用於網站服務、題目數位化、圖解製作與教學影片器材。加密貨幣贊助已開放，Line Pay 功能開發中。</p></div></section><section class="section"><div class="wrap"><div class="info-grid crypto-grid">
    <article class="info-card crypto-card crypto-combined"><span class="big-icon">⬡</span><h3>加密貨幣贊助</h3><p>EVM 與 Solana 網路皆可使用，請依照幣別與網路選擇正確地址。</p><div class="crypto-wallet-list"><div class="crypto-wallet"><b>EVM 網路</b><p>接受 ETH、USDT 與 USDC。</p><div class="network-list"><span>Arbitrum</span><span>Optimism</span><span>Base</span></div><div class="wallet-block"><small>收款地址</small><code>0x219Ac5c16dD7011Ff2c7a6DCF58F82Aa2F4aC88c</code><button class="copy-button" data-copy="0x219Ac5c16dD7011Ff2c7a6DCF58F82Aa2F4aC88c">複製地址</button></div></div><div class="crypto-wallet"><b>Solana</b><p>使用 Solana Mainnet，接受 SOL、USDT 與 USDC。</p><div class="network-list"><span>Solana Mainnet</span></div><div class="wallet-block"><small>收款地址</small><code>9LKm1a5gQjb2armKxAeURWWT4RsFgeGPBTESzH1Kikrm</code><button class="copy-button" data-copy="9LKm1a5gQjb2armKxAeURWWT4RsFgeGPBTESzH1Kikrm">複製地址</button></div></div></div></article>
...
function syncLearningUrl({ push = true } = {}) {
  ...
  if (learningState.subject === "數理資優班甄選") {
    if (learningState.giftedCity) params.set("city", learningState.giftedCity);
    if (learningState.giftedSchool) params.set("school", learningState.giftedSchool);
  } else if (learningState.subject === "科學班甄選考古題" && learningState.scienceLabSchool) params.set("school", learningState.scienceLabSchool);
...
function bindPageEvents() {
  ...
  document.querySelectorAll("[data-topic]").forEach(b => b.addEventListener("click", () => { if (b.dataset.topicSubject) learningState.subject = b.dataset.topicSubject; learningState.index = false; learningState.topic = b.dataset.topic; learningState.tab = "notes"; learningState.sidebarOpen = true; syncLearningUrl(); render({ preserveScroll: true }); }));
  ...
  document.querySelectorAll("[data-science-lab-school]").forEach(b => b.addEventListener("click", () => { learningState.subject = "科學班甄選考古題"; learningState.scienceLabSchool = b.dataset.scienceLabSchool; learningState.topic = learningState.scienceLabSchool; learningState.tab = "files"; learningState.sidebarOpen = true; syncLearningUrl(); render({ preserveScroll: true }); }));
